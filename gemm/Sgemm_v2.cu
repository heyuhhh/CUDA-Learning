...
void Sgemm_v2(
    ...
    for (int bk = 0; bk < (K + BK - 1) / BK; bk++) {
        ...
        for (int k = 0; k < BK; k++) {
            ...
            FLOAT4(r_a_comp[0]) = FLOAT4(s_a[k][ty * TM / 2]);
            FLOAT4(r_a_comp[4]) = FLOAT4(s_a[k][ty * TM / 2 + BM / 2]);
            FLOAT4(r_b_comp[0]) = FLOAT4(s_b[k][tx * TN / 2]);
            FLOAT4(r_b_comp[4]) = FLOAT4(s_b[k][tx * TN / 2 + BN / 2]);
            for (int m = 0; m < TM; m++) {
                for (int n = 0; n < TN; n++) {
...
int main() {
    ...
    void (*gpuGemm) (float *, float *, float *, const int, const int, const int) = Sgemm_v2;
...
float testError(void) {
    ...
    Sgemm_v2<<<gridDim, blockDim>>>(d_a, d_b, d_c, M, N, K);
